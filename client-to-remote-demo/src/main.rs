...
async fn main() -> Result<(), RTCError> {
    ...
    //                                         println!("Sending '{message}'");
    //                                         // result = data_channel_ref_2.send_text(message).await.map_err(Into::into);
    //                                         result = d2.send(&Bytes::from(message)).await.map_err(Into::into);
    //                                         packet_seq += 1;
    //                                     }
    //                                 };
    //                             }
    ...
    let session_description = serde_json::from_str::<RTCSessionDescription>(&desc_data).unwrap();
